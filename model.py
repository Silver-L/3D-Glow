...
def codec(hps):
    ...
    def encoder(z, objective):
        ...
        for i in range(hps.n_levels):
            ...
            if i < hps.n_levels-1:
                z, objective, _eps = split3d("pool"+str(i), z, objective=objective)
                eps.append(_eps)
    ...
    def decoder(z, eps=[None]*hps.n_levels, eps_std=None):
        for i in reversed(range(hps.n_levels)):
            if i < hps.n_levels-1:
                z = split3d_reverse("pool"+str(i), z, eps=eps[i], eps_std=eps_std)
...
def prior(name, y_onehot, hps):
    ...
    with tf.variable_scope(name):
        ...
        h = tf.zeros([tf.shape(y_onehot)[0]]+hps.top_shape[:3]+[2*n_z])
        if hps.learntop:
...
def model(sess, hps, train_iterator, test_iterator, data_init):
    ...
    with tf.name_scope('input'):
        X = tf.placeholder(
            tf.float32, [None, hps.image_size, hps.image_size, hps.image_size, 1], name='image')
    ...
    def f_sample(y, eps_std):
        with tf.variable_scope('model', reuse=True):
            ...
            z = Z.unsqueeze3d(z, 2)  # 4x4x4x8 -> 8x8x8x1
            x = postprocess(z)       # ???
...
def f_resnet(name, h, width, n_out=None):
    ...
    with tf.variable_scope(name):
        h = tf.nn.relu(Z.conv3d("l_1", h, width))
        h = Z.conv3d_zeros("l_2", h, n_out)
    return h


# Invertible 1x1 conv (modified by Zhihui)
@add_arg_scope
...
# by Zhihui
@add_arg_scope
def split3d_reverse(name, z, eps, eps_std):
    with tf.variable_scope(name):
        z1 = Z.unsqueeze3d(z)
        pz = split3d_prior(z1)
        if eps is not None:
...
@add_arg_scope
def split3d_prior(z):
    n_z2 = int(z.get_shape()[4])
    n_z1 = n_z2
    h = Z.conv3d_zeros("conv", z, 2 * n_z1)
